...
export function CaptureComposer({ draftKey, id, onSubmit, onClose, onSuccess, compact = false, autoFocus = false }: {
  ...
  return <div className={`capture-composer ${compact ? "capture-composer-compact" : ""}`}>
    <label className="sr-only" htmlFor={id}>Zapisz myśl, Działanie lub link</label>
    <Textarea id={id} rows={compact ? 2 : 5} placeholder="Zapisz myśl, Działanie lub link…" value={draft.value} onChange={(event) => draft.setValue(event.target.value)} onKeyDown={(event) => { if ((event.metaKey || event.ctrlKey) && event.key === "Enter") { event.preventDefault(); void submit(); } }} autoFocus={autoFocus} aria-invalid={Boolean(error)} aria-describedby={`${id}-feedback`} />
