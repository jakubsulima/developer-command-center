...
export class AppErrorBoundary extends Component<Props, State> {
  ...
  public render() {
    ...
    return <main className="app-error-screen" role="alert"><span className="loading-mark">!</span><h1>Coś przerwało działanie aplikacji</h1><p>Możesz wrócić do Startu albo przeładować aplikację. Dane zapisane wcześniej pozostają w Workspace.</p><div className="modal-actions"><button className="button button-secondary" onClick={() => { window.location.assign("/"); }}>Wróć do Startu</button><button className="button button-primary" onClick={() => window.location.reload()}>Przeładuj aplikację</button></div></main>;
  }
}
