...
const groupOrder: SearchGroup[] = ["Projekty", "Cele", "Zadania", "Wiedza", "Inbox"];
...
export function GlobalSearch({ id = "global-search", onNavigate }: { id?: string; onNavigate?: () => void }) {
  ...
  const results = useMemo<SearchResult[]>(() => {
    ...
    const matched = normalized ? candidates.filter((item) => normalizeSearchText(`${item.title} ${item.detail}`).includes(normalized)) : candidates;
    const perGroupLimit = normalized ? 6 : 3;
    return groupOrder.flatMap((group) => matched.filter((item) => item.group === group).slice(0, perGroupLimit));
  }, [normalized, state.actions, state.areas, state.goals, state.inbox, state.knowledge]);
  const groupedResults = useMemo(() => groupOrder.map((group) => ({ group, results: results.filter((result) => result.group === group) })).filter((section) => section.results.length), [results]);
  ...
  return <div ref={rootRef} className="global-search">
    <div className="search-wrap"><Search /><input id={id} role="combobox" aria-label="Szukaj w Projektach, Celach, Zadaniach i Wiedzy" aria-expanded={open} aria-controls={resultsId} aria-activedescendant={open && results[active] ? `${id}-${results[active].id}` : undefined} autoComplete="off" placeholder="Szukaj projektów, celów, zadań…" value={query} onFocus={() => setOpen(true)} onChange={(event) => { setQuery(event.target.value); setOpen(true); }} onKeyDown={(event) => { if (event.key === "ArrowDown") { event.preventDefault(); setActive((value) => results.length ? Math.min(results.length - 1, value + 1) : 0); } if (event.key === "ArrowUp") { event.preventDefault(); setActive((value) => Math.max(0, value - 1)); } if (event.key === "Enter" && results[active]) { event.preventDefault(); choose(results[active]); } if (event.key === "Escape") { setOpen(false); setQuery(""); event.currentTarget.blur(); } }} />{query ? <button type="button" aria-label="Wyczyść wyszukiwanie" onClick={() => setQuery("")}><X /></button> : <kbd><Keyboard /> K</kbd>}</div>
    {open && <div id={resultsId} className="search-results" role="listbox" aria-label="Wyniki wyszukiwania">{results.length ? <>
      <div className="search-results-summary" role="status"><span>{normalized ? "Wyniki wyszukiwania" : "Przegląd Workspace"}</span><strong>{results.length}</strong></div>
      {groupedResults.map((section) => {
        const sectionId = `${id}-${section.group.toLocaleLowerCase("pl")}-heading`;
        return <section className="search-result-group" role="group" aria-labelledby={sectionId} key={section.group}>
          <div className="search-result-group-heading" id={sectionId}><span>{section.group}</span><span>{section.results.length}</span></div>
          {section.results.map(({ icon: Icon, ...result }) => {
            const index = resultIndex++;
            return <button id={`${id}-${result.id}`} key={result.id} role="option" aria-selected={index === active} onMouseEnter={() => setActive(index)} onClick={() => choose({ ...result, icon: Icon })}><span className="search-result-icon"><Icon /></span><span className="search-result-copy"><strong>{result.title}</strong><small>{result.detail}</small></span></button>;
          })}
        </section>;
      })}
    </> : <div className="search-empty"><Search /><strong>Brak wyników</strong><p>Spróbuj krótszej frazy albo innej nazwy.</p></div>}</div>}
  </div>;
}
