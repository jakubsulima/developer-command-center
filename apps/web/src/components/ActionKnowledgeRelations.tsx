...
export function ActionKnowledgeRelations({ action, navigation }: { action: GoalAction; navigation?: { breadcrumbs: NavigationBreadcrumb[]; returnTo: string; returnLabel: string } }) {
  ...
  const create = () => mutation.run(`action-relations:${action.id}`, async () => { if (!form.title.trim() || (knowledgeKindGuidance[form.kind].detailRequired && !form.detail.trim())) return; await createKnowledge({ kind: form.kind, title: form.title, detail: form.detail, sourceUrl: form.sourceUrl || undefined, relations: [{ meaning: form.meaning, target: { actionId: action.id } }] }); setOpen(false); setForm({ kind: "note", title: "", detail: "", sourceUrl: "", meaning: "reference" }); });
