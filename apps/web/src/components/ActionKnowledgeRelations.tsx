...
export function ActionKnowledgeRelations({ action }: { action: GoalAction }) {
  ...
  return <Panel className="action-relations-panel" aria-labelledby={`action-relations-${action.id}`}>
    <div className="action-relations-heading"><h3 id={`action-relations-${action.id}`}><Link2 />Wiedza Działania</h3><Button variant="ghost" className="action-relations-toggle" aria-expanded={expanded} aria-controls={`action-relations-content-${action.id}`} onClick={() => setExpanded((current) => !current)}>{links.length ? `Wiedza · ${links.length}` : "Dodaj wiedzę"}</Button></div>
