...
import { goalKindLabels, goalStatusLabels } from "../domain/labels";
...
export function GoalDetailPage() {
  ...
  useEffect(() => { const id = searchParams.get("action"); if (id) requestAnimationFrame(() => document.querySelector<HTMLElement>(`[data-action-id="${CSS.escape(id)}"]`)?.focus()); }, [searchParams, state.actions]);
  ...
  const areaName = state.areas.find((area) => area.id === goal.areaId)?.name ?? "Bez Projektu";
  ...
  return <AppShell>
    ...
    <section className="goal-overview">
      ...
      <div className="goal-health-grid"><div><span><ListTodo />Następny krok</span><strong>{nextAction?.title ?? "Wybierz następne Działanie"}</strong></div><div><span><Target />Kryteria</span><strong>{criteria.length ? `${completedCriteria} z ${criteria.length} spełnione` : "Brak kryteriów"}</strong></div><div><span><Flag />Otwarte Działania</span><strong>{openActions}</strong></div><div><span><Circle />Stan</span><strong>{goalStatusLabels[goal.status]}</strong></div></div>
    </section>
    ...
    <div className="goal-detail-grid">
      <div className="detail-main">
        <Panel>
          <div className="section-heading"><div><h2>Działania</h2><span>Każdy krok można wykonać bez uruchamiania sesji.</span></div></div>
          <div className="action-list">{actions.map((action) => <div className={`goal-action ${action.status}`} key={action.id} data-action-id={action.id} tabIndex={-1}>
            <ActionPrimaryControls action={action} busy={actionMutation.isBusy(`goal-action:${action.id}`)} onToggleComplete={() => void (action.status === "completed" ? restoreCompletedAction(action) : complete(action.id))} onSetNext={() => void actionMutation.run(`goal-action:${action.id}`, () => setNextAction(goal.id, action.id))} onMore={() => setActionMenuId(action.id)} />
            <div className="action-copy"><strong>{action.title}</strong><small>{action.detail || (action.scheduledFor ? `Termin: ${action.scheduledFor}` : "Bez terminu")}</small>{action.blocker && <em><LockKeyhole />{action.blocker}</em>}{action.checklist.length ? <div className="action-checklist"><small>Checklista: {action.checklist.filter((entry) => entry.completed).length}/{action.checklist.length}</small>{action.checklist.map((entry) => <label key={entry.id}><input type="checkbox" checked={entry.completed} disabled={actionMutation.isBusy(`goal-action:${action.id}`) || ["completed", "cancelled", "skipped"].includes(action.status)} onChange={() => void actionMutation.run(`goal-action:${action.id}`, () => updateAction(action.id, { checklist: action.checklist.map((candidate) => candidate.id === entry.id ? { ...candidate, completed: !candidate.completed } : candidate) }))} />{entry.title}</label>)}</div> : null}{state.knowledgeLinks.filter((link) => link.actionId === action.id).map((link) => state.knowledge.find((item) => item.id === link.knowledgeItemId)?.title).filter(Boolean).length ? <small>Materiały: {state.knowledgeLinks.filter((link) => link.actionId === action.id).map((link) => state.knowledge.find((item) => item.id === link.knowledgeItemId)?.title).filter(Boolean).join(" · ")}</small> : null}{actionMutation.error(`goal-action:${action.id}`) ? <p className="inline-mutation-error" role="alert">{actionMutation.error(`goal-action:${action.id}`)} <button type="button" onClick={() => void actionMutation.retry(`goal-action:${action.id}`)?.()}>Spróbuj ponownie</button></p> : null}</div>
            {action.isNext && <Badge tone="info">Następne</Badge>}
            <div className="action-menu">
              ...
              <Button variant="ghost" loading={actionMutation.isBusy(`goal-action:${action.id}`)} onClick={() => { const previous = action.pinnedToToday; void actionMutation.run(`goal-action:${action.id}`, async () => { await updateAction(action.id, { pinnedToToday: !previous }); notifyUndo({ message: previous ? "Odpięto od Dzisiaj." : "Przypięto do Dzisiaj.", undo: () => updateAction(action.id, { pinnedToToday: previous }) }); }); }}>{action.pinnedToToday ? "Odepnij" : "Przypnij"}</Button>
