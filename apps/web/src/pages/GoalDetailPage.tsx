...
export function GoalDetailPage() {
  ...
  const renderAction = (action: typeof actions[number], featured = false) => featured ? <><div className={`goal-featured-action ${action.status}`} key={action.id} data-action-id={action.id} data-navigation-card-id={navigationCardId("action", action.id)} tabIndex={-1}>
    ...
    <div className="action-copy"><NavigationLink to={`/actions/${encodeURIComponent(action.id)}`} breadcrumbs={breadcrumbs} returnTo={locationAddress(location)} returnLabel={`Cel: ${goal.title}`} sourceCardId={navigationCardId("action", action.id)}><strong>{action.title}</strong></NavigationLink><small>{action.scheduledFor ? `Termin: ${action.scheduledFor}` : action.status === "in_progress" ? "W toku" : "Gotowe"}</small></div>
