...
import { actionStatusLabels, goalKindLabels, progressKindLabels } from "../domain/labels";
...
export function GoalDetailPage() {
  ...
  const blockerDraft = usePersistentDraft("action-blocker", { content: "", reviewOn: "" }, 450, { targetId: blockActionId ?? "new", enabled: Boolean(blockActionId) });
  const blocker = blockerDraft.value.content;
  const blockerReviewOn = blockerDraft.value.reviewOn ?? "";
  ...
  const progressContentRef = useRef<HTMLTextAreaElement>(null);
  useEffect(() => { if (progressDraft.restored) setProgressOpen(true); }, [progressDraft.restored]);
  ...
  const visibleCriteria = criteriaExpanded ? criteria : criteria.slice(0, 3);
  ...
  const changeActionStatus = (action: typeof actions[number], status: ProjectActionStatus, blocker?: string, reviewOn?: string | null) => actionMutation.run(`goal-action-status:${action.id}`, async () => {
  ...
  const renderAction = (action: typeof actions[number], featured = false) => featured ? <><div className={`goal-featured-action ${action.status}`} key={action.id} data-action-id={action.id} data-navigation-card-id={navigationCardId("action", action.id)} tabIndex={-1}>
    ...
    <div className="action-menu">
      ...
      {['ready', 'in_progress', 'testing'].includes(action.status) && <Button variant="ghost" aria-label={`Zablokuj: ${action.title}`} onClick={() => { setDialogError(""); setBlockActionId(action.id); }}><LockKeyhole /></Button>}
  ...
  return <AppShell appearance="focus-detail" addAction={{ label: "Nowe Działanie", shortLabel: "Działanie", ariaLabel: `Dodaj Działanie do Celu ${goal.title}`, quickAdd: { mode: "action", goalId: goal.id, areaId: goal.areaId, pinnedToToday: false, draftKey: `goal-${goal.id}` } }}>
    ...
    <section className="goal-overview">
      <div className="goal-overview-main"><h1>{goal.title}</h1>{project ? <NavigationLink className="goal-project-link" to={`/projects/${encodeURIComponent(project.id)}`} breadcrumbs={breadcrumbs} returnTo={locationAddress(location)} returnLabel={`Cel: ${goal.title}`}><FolderKanban />{project.name}<ChevronDown /></NavigationLink> : null}<p><span className="goal-outcome-label">Oczekiwany rezultat</span>{goal.outcome}</p></div>
    </section>
    ...
    <Panel className="goal-next-step" aria-labelledby="goal-next-step-title">
      <div className="section-heading"><div><h2 id="goal-next-step-title">Następne Działanie</h2></div></div>
      {nextAction ? <div className="next-action-card">{renderAction(nextAction, true)}</div> : <div className="next-action-empty"><span>Wybierz krok, od którego chcesz zacząć.</span><Button variant="primary" onClick={() => { if (actionsSectionRef.current) actionsSectionRef.current.open = true; requestAnimationFrame(() => { actionsSectionRef.current?.scrollIntoView({ block: "nearest" }); (remainingActions.length ? actionsSectionRef.current?.querySelector<HTMLButtonElement>(".goal-action button") : newActionInputRef.current)?.focus(); }); }}>{remainingActions.length ? "Wybierz następne działanie" : "Dodaj działanie"}</Button></div>}
    </Panel>
    <section className="goal-criteria-surface" aria-labelledby="goal-criteria-title"><div className="goal-section-heading"><div><h2 id="goal-criteria-title">Kryteria sukcesu</h2></div><strong>{criteria.length ? `${completedCriteria} z ${criteria.length}` : ""}</strong></div>{criteria.length ? <><ul className="plain-list">{visibleCriteria.map((item) => { const key = `criterion:${item.id}`; return <li key={item.id}><label className="criterion-check"><input type="checkbox" checked={item.completed} disabled={actionMutation.isBusy(key)} onChange={() => void actionMutation.run(key, () => updateGoal(goal.id, { criteria: criteria.map((criterion) => criterion.id === item.id ? { ...criterion, completed: !criterion.completed } : criterion) }))} />{item.completed ? <Check /> : <Circle />}<span>{item.title}</span></label>{actionMutation.error(key) ? <p className="inline-mutation-error" role="alert">{actionMutation.error(key)} <button type="button" onClick={() => void actionMutation.retry(key)?.()}>Spróbuj ponownie</button></p> : null}</li>; })}</ul>{criteria.length > 3 ? <button type="button" className="show-more-link" onClick={() => setCriteriaExpanded((current) => !current)}>{criteriaExpanded ? "Pokaż mniej" : `Pokaż wszystkie (${criteria.length})`}</button> : null}</> : <div className="criteria-empty"><p className="muted-copy">Nie określono jeszcze kryteriów sukcesu.</p><Button variant="ghost" onClick={openGoalEditor}><Plus />Dodaj kryteria</Button></div>}</section>
    <Panel className="goal-secondary" aria-label="Szczegóły Celu">
      <div className="goal-secondary-list">
        <details className="goal-secondary-section" ref={actionsSectionRef}>
          <summary><span className="goal-secondary-icon"><ListTodo /></span><span className="goal-secondary-label"><strong>Pozostałe Działania</strong><small>Otwarte kroki poza następnym, dodawanie i historia</small></span><span className="goal-secondary-count">{remainingActions.length}</span><ChevronDown /></summary>
          <div className="goal-secondary-content">
            <div className="action-list">{remainingActions.length ? remainingActions.map((action) => renderAction(action)) : <p className="muted-copy action-list-empty">Brak pozostałych otwartych Działań.</p>}</div>
            ...
            {historicalActions.length ? <details className="goal-action-history"><summary><History />Historia Działań <span>{historicalActions.length}</span></summary><div className="action-list">{historicalActions.map((action) => renderAction(action))}</div></details> : null}
          </div>
        </details>
        <details className="goal-secondary-section">
          <summary><span className="goal-secondary-icon"><History /></span><span className="goal-secondary-label"><strong>Historia postępu</strong><small>Decyzje, rezultaty i wcześniejsze aktualizacje</small></span><span className="goal-secondary-count">{updates.length}{progressPage.hasNextPage ? "+" : ""}</span><ChevronDown /></summary>
    ...
    <Modal open={Boolean(blockActionId)} closeDisabled={dialogSaving} title="Co blokuje Działanie?" onClose={() => setBlockActionId(undefined)}><textarea aria-label="Powód blokady" rows={3} value={blocker} onChange={(event) => blockerDraft.setValue((current) => ({ ...current, content: event.target.value }))} /><label className="field-label" htmlFor="goal-blocker-review-on">Sprawdź ponownie <span className="optional-label">opcjonalnie</span></label><input id="goal-blocker-review-on" type="date" value={blockerReviewOn} onChange={(event) => blockerDraft.setValue((current) => ({ ...current, reviewOn: event.target.value }))} /><p className="muted-copy">Data powrotu do blokady nie zmienia terminu wykonania Działania.</p>{dialogError ? <p className="auth-message error" role="alert">{dialogError}</p> : null}<div className="modal-actions"><DraftStatus status={blockerDraft.status} errorMessage={blockerDraft.errorMessage} onRetry={() => void blockerDraft.retry()} onCopy={() => void navigator.clipboard?.writeText(blocker)} />{blockerDraft.dirty ? <Button type="button" variant="ghost" onClick={blockerDraft.discard}>Odrzuć szkic</Button> : null}<Button disabled={dialogSaving} onClick={() => setBlockActionId(undefined)}>Anuluj</Button><Button variant="primary" loading={dialogSaving} disabled={!blocker.trim()} onClick={() => blockActionId && void runDialog(() => setActionStatus(blockActionId, "blocked", blocker, actions.find((action) => action.id === blockActionId)?.version, blockerReviewOn || null), () => { blockerDraft.clear(); setBlockActionId(undefined); })}>Zapisz blokadę</Button></div></Modal>
    {(() => { const action = actions.find((candidate) => candidate.id === editActionId); const key = action ? `goal-action-edit:${action.id}` : ""; return <ActionEditDialog action={action} open={Boolean(action)} busy={Boolean(key && actionMutation.isBusy(key))} error={key ? actionMutation.error(key) : undefined} onClose={() => setEditActionId(undefined)} onSave={(value, expectedVersion) => action ? saveActionEdit(action, value, expectedVersion) : false} />; })()}
