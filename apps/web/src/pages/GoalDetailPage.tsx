...
export function GoalDetailPage() {
  ...
  const changeActionStatus = (action: typeof actions[number], status: ProjectActionStatus, blocker?: string) => actionMutation.run(`goal-action-status:${action.id}`, async () => {
  ...
  const renderAction = (action: typeof actions[number], featured = false) => featured ? <><div className={`goal-featured-action ${action.status}`} key={action.id} data-action-id={action.id} data-navigation-card-id={navigationCardId("action", action.id)} tabIndex={-1}>
    <div className="action-copy"><NavigationLink to={`/actions/${encodeURIComponent(action.id)}`} breadcrumbs={breadcrumbs} returnTo={locationAddress(location)} returnLabel={`Cel: ${goal.title}`} sourceCardId={navigationCardId("action", action.id)}><strong>{action.title}</strong></NavigationLink>{action.recurringTemplateId ? <ActionOriginMarker action={action} /> : null}<small>{action.scheduledFor ? `Termin: ${action.scheduledFor}` : "Bez terminu"}</small></div>
    <div className="action-row-controls"><ActionStatusIconTrigger action={action} disabled={actionMutation.isBusy(`goal-action-status:${action.id}`)} onClick={() => setStatusActionId(action.id)} /><ActionOverflowButton action={action} busy={actionMutation.isBusy(`goal-action:${action.id}`)} onClick={() => setActionMenuId(action.id)} /></div>
      ...
      <div className="goal-secondary-list">
        <details className="goal-secondary-section" ref={actionsSectionRef}>
          ...
          <div className="goal-secondary-content">
            <div className="action-list">{remainingActions.length ? remainingActions.map((action) => renderAction(action)) : <p className="muted-copy action-list-empty">Brak innych otwartych Działań.</p>}</div>
