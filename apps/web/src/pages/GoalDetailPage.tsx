...
export function GoalDetailPage() {
  ...
  return <AppShell addAction={{ label: "Nowe Działanie", shortLabel: "Działanie", ariaLabel: `Dodaj Działanie do Celu ${goal.title}`, onClick: openActionComposer }}>
    ...
    <Modal open={Boolean(editAction)} closeDisabled={dialogSaving} title="Edytuj Działanie" onClose={() => setEditAction(undefined)}>{editAction ? <>
      <label className="field-label" htmlFor="edit-action-name">Nazwa</label><input id="edit-action-name" value={editActionDraft.value.title} onChange={(event) => editActionDraft.setValue((current) => ({ ...current, title: event.target.value }))} autoFocus />
