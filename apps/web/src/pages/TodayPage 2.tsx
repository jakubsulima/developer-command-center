...
function TodayActionRow({ action, context, complete, reschedule, skip, togglePin, openMore, busy, error, retry }: {
  ...
  return <div className="today-action">
    ...
    <div className="today-row-actions">
      ...
      {action.goalId ? <Link aria-label={`Otwórz Cel: ${action.title}`} to={routeForEntity({ type: "goal", id: action.goalId })}><ChevronRight /></Link> : null}
    </div>
...
export function TodayPage() {
  ...
  useEffect(() => {
    ...
    if (actionId) requestAnimationFrame(() => { const target = document.querySelector<HTMLElement>(`[data-action-id="${CSS.escape(actionId)}"]`); target?.scrollIntoView?.({ block: "center", behavior: "auto" }); target?.focus(); });
    else if (seriesId) requestAnimationFrame(() => { const target = document.querySelector<HTMLElement>(`[data-series-id="${CSS.escape(seriesId)}"]`); target?.scrollIntoView?.({ block: "center", behavior: "auto" }); target?.focus(); });
  ...
  return <AppShell>
    <PageHeading title="Dzisiaj" eyebrow={new Intl.DateTimeFormat("pl-PL", { weekday: "long", day: "numeric", month: "long" }).format(new Date())} action={hasTodayWork ? <div className="button-row"><Button variant="primary" onClick={() => setActionOpen(true)}><Plus />Dodaj Działanie</Button><Button onClick={() => setRecurringOpen(true)}><Repeat2 />Nowe cykliczne</Button></div> : undefined} />
    {sections.today.length || sections.overdue.length ? <div className="today-layout">
      <div className="today-main">
        ...
        {reviewAction && <Panel className="review-signals"><div className="section-heading"><h2>Sygnały do przeglądu</h2><span>{reviewSignals.length + state.inbox.filter((item) => item.status === "unprocessed").length} spraw</span></div>{reviewSignals.map((signal) => <Link key={signal.id} to={routeForEntity({ type: "goal", id: signal.goalId })}><span><strong>{signal.title}</strong><small>{signal.reason}</small></span><ChevronRight /></Link>)}{state.inbox.filter((item) => item.status === "unprocessed").slice(0, 3).map((item) => <Link key={item.id} to={routeForEntity({ type: "inbox", id: item.id, status: item.status })}><span><strong>{item.content}</strong><small>Pilny Inbox</small></span><ChevronRight /></Link>)}{!reviewSignals.length && !state.inbox.some((item) => item.status === "unprocessed") && <p className="muted-copy">Brak sygnałów — możesz po prostu ukończyć przegląd.</p>}</Panel>}
    ...
    </div> : <EmptyState icon={<CalendarClock />} title="Dzisiaj jest spokojnie" detail="Dodaj zwykłe Działanie albo utwórz lekką serię cykliczną — bez timera i bez presji." action={<div className="empty-primary-action"><Button variant="primary" onClick={() => setActionOpen(true)}><Plus />Dodaj Działanie</Button><button className="text-action" type="button" onClick={() => setRecurringOpen(true)}><Repeat2 />Nowe cykliczne</button></div>} />}
    ...
    <Modal open={Boolean(actionMenuId)} closeDisabled={Boolean(actionMenuId && actionMutation.isBusy(`today:${actionMenuId}`))} title="Działanie — więcej opcji" onClose={() => setActionMenuId(undefined)}>{(() => {
      ...
      return <div className="mobile-action-sheet">
        ...
        {action.goalId ? <Link className="button button-secondary" to={routeForEntity({ type: "goal", id: action.goalId })} onClick={() => setActionMenuId(undefined)}>Otwórz Cel</Link> : null}
        {actionMutation.error(key) ? <p className="inline-mutation-error" role="alert">{actionMutation.error(key)} <button type="button" onClick={() => void actionMutation.retry(key)?.()}>Spróbuj ponownie</button></p> : null}
      </div>;
    })()}</Modal>
    ...
    <Modal open={recurringOpen} closeDisabled={recurringSaving} title={editingTemplateId ? "Edytuj serię cykliczną" : "Nowe Działanie cykliczne"} onClose={() => { setRecurringOpen(false); setEditingTemplateId(undefined); }}>
      <form className="guided-form" onSubmit={submitRecurring}>
        ...
        <div className="preset-library"><span>Zacznij od gotowej propozycji</span><div>{recurringPresets.map((preset) => <button type="button" aria-pressed={form.title === preset.name} key={preset.name} onClick={() => setForm((current) => ({ ...current, title: preset.name, unit: preset.unit, checklist: preset.checklist }))}>{preset.name}</button>)}</div></div>
