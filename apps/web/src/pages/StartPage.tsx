...
function StartActionRow({ action, context, relationSummary, complete, reschedule, skip, togglePin, setNext, openMore, busy, error, retry }: {
  action: GoalAction;
  ...
  return <div className="today-action">
    ...
    <div className="action-copy"><strong>{action.title}</strong><small>{context.to !== "/" ? <Link to={context.to}>{describeActionContext(context)}</Link> : describeActionContext(context)}{action.recurringTemplateId ? <> · <Repeat2 /> cykliczne</> : null}</small>{relationSummary.length ? <small className="action-relation-summary">{relationSummary.join(" · ")}</small> : null}</div>
...
export function StartPage() {
  ...
  const renderAction = (action: GoalAction) => { const relationSummary = state.knowledgeLinks.filter((link) => link.actionId === action.id).map((link) => `${link.meaning === "result" ? "Rezultat" : link.meaning === "decision" ? "Decyzja" : link.meaning === "material" ? "Materiał" : "Pozostałe"}: ${state.knowledge.find((item) => item.id === link.knowledgeItemId)?.title ?? "Niedostępna Wiedza"}`); return <div key={action.id} data-action-id={action.id} tabIndex={-1}><StartActionRow action={action} context={contextFor(action)} relationSummary={relationSummary} complete={complete} reschedule={reschedule} skip={skip} togglePin={togglePin} setNext={setNext} openMore={setActionMenuId} busy={mutation.isBusy(`start:${action.id}`)} error={mutation.error(`start:${action.id}`)} retry={mutation.retry(`start:${action.id}`)} /></div>; };
