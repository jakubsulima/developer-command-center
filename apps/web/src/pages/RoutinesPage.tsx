...
import { polishCount } from "../domain/labels";
...
export function RoutinesPage() {
  ...
  return <AppShell addAction={{ label: "Nowa Rutyna", shortLabel: "Rutyna", ariaLabel: "Dodaj nową Rutynę", active: formOpen && !editingTemplateId, quickAdd: { mode: "routine", pinnedToToday: false, draftKey: "routines-list" }, onClick: openNew }}>
    ...
    {routines.length ? <div className="routine-grid">{routines.map((routine) => {
      ...
      const rangeLabel = `Od ${formatDate(routine.startsOn, state.workspaceTimezone)}${routine.rule.endsOn ? ` do ${formatDate(routine.rule.endsOn, state.workspaceTimezone)}` : " · bez daty końcowej"}`;
      const missedLabel = routine.missedPolicy === "skip_missed" ? "Pomiń stare terminy" : "Zachowaj jedno zaległe";
      return <Panel className="routine-card" key={routine.id}>
        ...
        <dl className="routine-details">
          ...
          <div><dt><CalendarDays />Zakres</dt><dd>{rangeLabel}</dd></div>
          <div><dt><ShieldCheck />Po przerwie</dt><dd>{missedLabel}</dd></div>
        </dl>
        ...
        <details className="routine-mobile-details"><summary>Więcej o rutynie</summary><dl><div><dt>Zakres</dt><dd>{rangeLabel}</dd></div><div><dt>Po przerwie</dt><dd>{missedLabel}</dd></div>{occurrences.length > 1 ? <div><dt>Kolejne terminy</dt><dd>{occurrences.slice(1).map((date) => formatDate(date, state.workspaceTimezone, true)).join(", ")}</dd></div> : null}</dl></details>
