...
export function ProjectDetailPage() {
  ...
  return <AppShell addAction={addAction}>
    ...
    <div className="project-detail-categories"><div className="project-card-categories">{(state.projectCategories ?? []).filter((category) => project.categoryIds?.includes(category.id)).map((category) => <span key={category.id} className="project-category-chip"><span style={{ backgroundColor: category.color }} />{category.name}</span>)}</div><Button variant="ghost" onClick={openProjectEditor}>Zmień kategorie</Button></div>
    {view === "overview" ? <>
