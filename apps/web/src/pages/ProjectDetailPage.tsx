...
import { Archive, ArrowRight, BookOpen, ChevronDown, FileText, Flag, FolderKanban, History, Lightbulb, ListChecks, Pencil, Plus, RotateCcw, Trash2 } from "lucide-react";
...
import { actionStatusLabels, goalStatusLabels, knowledgeKindLabels } from "../domain/labels";
...
export function ProjectDetailPage() {
  ...
  const actionFilter: ProjectHistoryFilter = searchParams.get("actions") === "history" ? "history" : "current";
  ...
  const setActionFilter = (next: ProjectHistoryFilter) => updateProjectParam("actions", next, "current");
  ...
  const displayedActions = actionFilter === "current" ? openActions : historicalActions;
  ...
  return <AppShell>
    <ContextNavigation current={{ label: project.name, to: projectUrl }} fallbackBreadcrumbs={[{ label: "Projekty", to: "/projects" }]} fallbackReturnTo="/projects" fallbackReturnLabel="Wszystkie Projekty" />
    ...
    <div className="project-summary-strip">
      <span><Flag /><strong>{currentGoals.length}</strong><small>Bieżące Cele</small></span>
    ...
    {view === "overview" ? <details className="project-flow-guide"><summary><span className="project-flow-guide-label"><span><Lightbulb /></span><span><strong>Pętla pracy — wskazówka</strong><small>Jak przejść od kierunku Projektu do zachowanej Wiedzy</small></span></span><span className="project-flow-guide-action">Pokaż <ChevronDown /></span></summary><section className="project-flow" aria-labelledby="project-flow-title"><div className="project-flow-heading"><div><span className="eyebrow">Pętla pracy</span><h2 id="project-flow-title">Od kierunku do wiedzy</h2></div><p>Każdy krok zostawia kontekst, który można wykorzystać ponownie.</p></div><div className="project-flow-steps"><div><span>1</span><small>Projekt</small><strong>{project.name}</strong><p>Kierunek i wspólny kontekst</p></div><ArrowRight /><button type="button" onClick={() => setView("goals")}><span>2</span><small>Cele</small><strong>{activeGoals.length ? activeGoalsLabel : "Ustal rezultat"}</strong><p>Po czym poznasz sukces</p></button><ArrowRight /><button type="button" onClick={() => setView("actions")}><span>3</span><small>Działanie</small><strong>{nextAction?.title ?? "Wybierz następny krok"}</strong><p>Co konkretnie robisz teraz</p></button><ArrowRight /><button type="button" onClick={() => setView("knowledge")}><span>4</span><small>Rezultat / wiedza</small><strong>{insights.length ? insightsLabel : "Zapisz rezultat"}</strong><p>Co zostaje na przyszłość</p></button></div></section></details> : null}

    <div className="project-sections">
      {visibleSections.includes("goals") ? <section aria-labelledby="project-goals-title"><div className="section-heading project-section-heading"><div><h2 id="project-goals-title">Cele</h2><span>{goalFilter === "current" ? "Bieżące rezultaty do wykonania w tym Projekcie" : "Osiągnięte i porzucone Cele"}</span></div><div className="project-section-actions"><div className="project-history-filter" role="group" aria-label="Widok Celów"><button type="button" aria-pressed={goalFilter === "current"} onClick={() => setGoalFilter("current")}>Bieżące <span>{currentGoals.length}</span></button><button type="button" aria-pressed={goalFilter === "history"} onClick={() => setGoalFilter("history")}><History />Historia <span>{historicalGoals.length}</span></button></div><Button className="project-section-add" aria-label="Dodaj Cel" onClick={() => setDialog("goal")}><Plus /><span>Dodaj Cel</span></Button></div></div>
        {displayedGoals.length ? <div className="project-entity-list">{displayedGoals.map((goal) => <Panel className={`${entityCardVariants({ density: "compact" })} entity-card`} key={goal.id} data-navigation-card-id={navigationCardId("goal", goal.id)} tabIndex={-1}><Flag /><span><strong className="line-clamp-2">{goal.title}</strong><small className="line-clamp-2">{goal.outcome}</small></span><Badge>{goalStatusLabels[goal.status]}</Badge><NavigationLink className="button button-ghost entity-card-open" to={routeForEntity({ type: "goal", id: goal.id })} breadcrumbs={projectBreadcrumbs} returnTo={projectNavigation.returnTo} returnLabel={projectNavigation.returnLabel} sourceCardId={navigationCardId("goal", goal.id)} aria-label={`Otwórz Cel: ${goal.title}`}><ArrowRight /></NavigationLink></Panel>)}</div> : <EmptyState icon={goalFilter === "history" ? <History /> : <Flag />} title={goalFilter === "history" ? "Historia Celów jest pusta" : "Brak bieżących Celów"} detail={goalFilter === "history" ? "Osiągnięte i porzucone Cele pojawią się tutaj." : historicalGoals.length ? "Zakończone Cele są zachowane w historii." : "Dodaj pierwszy, konkretny rezultat w ramach tego Projektu."} action={goalFilter === "current" && historicalGoals.length ? <Button onClick={() => setGoalFilter("history")}><History />Pokaż historię</Button> : goalFilter === "current" ? <Button onClick={() => setDialog("goal")}><Plus />Dodaj Cel</Button> : undefined} />}
      </section> : null}

      {visibleSections.includes("actions") ? <section aria-labelledby="project-actions-title"><div className="section-heading project-section-heading"><div><h2 id="project-actions-title">Działania</h2><span>{actionFilter === "current" ? "Otwarte kroki — z Celu albo bezpośrednio z Projektu" : "Ukończone, pominięte i anulowane Działania"}</span></div><div className="project-section-actions"><div className="project-history-filter" role="group" aria-label="Widok Działań"><button type="button" aria-pressed={actionFilter === "current"} onClick={() => setActionFilter("current")}>Otwarte <span>{openActions.length}</span></button><button type="button" aria-pressed={actionFilter === "history"} onClick={() => setActionFilter("history")}><History />Historia <span>{historicalActions.length}</span></button></div><Button className="project-section-add" aria-label="Dodaj Działanie" onClick={() => setDialog("action")}><Plus /><span>Dodaj Działanie</span></Button></div></div>
        {displayedActions.length ? <div className="project-action-list">{displayedActions.map((action) => <div className={`project-action-row ${action.status}`} key={action.id} data-action-id={action.id} data-navigation-card-id={navigationCardId("action", action.id)} tabIndex={-1}>
          ...
          <div className="action-copy"><div className="action-title-row"><NavigationLink className="project-action-title" to={routeForEntity({ type: "action", id: action.id })} breadcrumbs={projectBreadcrumbs} returnTo={projectNavigation.returnTo} returnLabel={projectNavigation.returnLabel} sourceCardId={navigationCardId("action", action.id)} aria-label={`Otwórz Działanie: ${action.title}`}><strong>{action.title}</strong></NavigationLink>{action.isNext ? <span className="action-next-badge">Następne</span> : null}</div><small>{action.goalId ? goals.find((goal) => goal.id === action.goalId)?.title : "Działanie Projektu"}{action.scheduledFor ? ` · ${action.scheduledFor}` : ""}</small>{action.status === "blocked" ? <Badge tone="danger">Zablokowane</Badge> : historicalActions.includes(action) ? <Badge tone="neutral">{actionStatusLabels[action.status]}</Badge> : null}</div>
        </div>)}</div> : <EmptyState icon={actionFilter === "history" ? <History /> : <ListChecks />} title={actionFilter === "history" ? "Historia Działań jest pusta" : "Brak otwartych Działań"} detail={actionFilter === "history" ? "Ukończone, pominięte i anulowane Działania pojawią się tutaj." : historicalActions.length ? "Zakończone Działania są zachowane w historii." : "Dodaj pojedynczy krok lub utwórz go wewnątrz Celu."} action={actionFilter === "current" && historicalActions.length ? <Button onClick={() => setActionFilter("history")}><History />Pokaż historię</Button> : actionFilter === "current" ? <Button onClick={() => setDialog("action")}><Plus />Dodaj Działanie</Button> : undefined} />}
      </section> : null}

      {visibleSections.includes("knowledge") ? <section aria-labelledby="project-knowledge-title"><div className="section-heading project-section-heading"><div><h2 id="project-knowledge-title">Wiedza</h2><span>Notatki, materiały i decyzje zachowane przy Projekcie</span></div><div className="project-section-actions project-section-actions-single"><Button className="project-section-add" aria-label="Dodaj Wiedzę" onClick={() => setDialog("knowledge")}><Plus /><span>Dodaj Wiedzę</span></Button></div></div>
        {knowledge.length ? <div className="project-entity-list">{knowledge.map((item) => <Panel className="entity-card" key={item.id} data-navigation-card-id={navigationCardId("knowledge", item.id)} tabIndex={-1}><FileText /><span><strong className="line-clamp-2">{item.title}</strong><small className="line-clamp-2">{item.detail || knowledgeKindLabels[item.type]}</small></span><Badge>{knowledgeKindLabels[item.type]}</Badge><NavigationLink className="button button-ghost entity-card-open" to={routeForEntity({ type: "knowledge", id: item.id })} breadcrumbs={projectBreadcrumbs} returnTo={projectNavigation.returnTo} returnLabel={projectNavigation.returnLabel} sourceCardId={navigationCardId("knowledge", item.id)} aria-label={`Otwórz Wiedzę: ${item.title}`}><ArrowRight /></NavigationLink></Panel>)}</div> : <EmptyState icon={<BookOpen />} title="Brak Wiedzy" detail="Zapisz materiał, decyzję albo notatkę, która ma zostać w tym Projekcie." action={<Button onClick={() => setDialog("knowledge")}><Plus />Dodaj Wiedzę</Button>} />}
      </section> : null}
    </div>
