...
import { Archive, ArrowRight, BookOpen, ChevronDown, FileText, Flag, FolderKanban, History, Lightbulb, ListChecks, MoreHorizontal, Pencil, RotateCcw, Trash2 } from "lucide-react";
...
import { ActionStatusDialog, ActionStatusIconTrigger, type ProjectActionStatus } from "../components/ActionStatusControls";
...
export function ProjectDetailPage() {
  ...
  return <AppShell addAction={addAction}>
    ...
    <div className="project-sections">
      ...
      {visibleSections.includes("actions") ? <section aria-labelledby="project-actions-title"><div className="section-heading project-section-heading"><div><h2 id="project-actions-title">Działania</h2><span>{actionFilter === "current" ? "Otwarte kroki — z Celu albo bezpośrednio z Projektu" : "Ukończone, pominięte i anulowane Działania"}</span></div>{actionFilter === "current" && historicalActions.length ? <div className="project-section-actions project-section-actions-single"><details className="project-more-menu project-section-more-menu"><summary aria-label="Więcej opcji Działań" title="Więcej opcji Działań"><MoreHorizontal /><span className="sr-only">Więcej opcji Działań</span></summary><div><Button onClick={(event) => { closeProjectMenu(event.currentTarget); setActionFilter("history"); }}><History />Pokaż historię <span className="project-menu-count">{historicalActions.length}</span></Button></div></details></div> : null}</div>
        {actionFilter === "history" ? <div className="project-history-banner"><span><History /><span><strong>Historia Działań</strong><small>{historicalActions.length} {historicalActions.length === 1 ? "zapisane Działanie" : "zapisanych Działań"}</small></span></span><Button variant="ghost" onClick={() => setActionFilter("current")}><RotateCcw />Otwarte</Button></div> : null}
        ...
        </div>)}</div> : <EmptyState icon={actionFilter === "history" ? <History /> : <ListChecks />} title={actionFilter === "history" ? "Historia Działań jest pusta" : "Brak otwartych Działań"} detail={actionFilter === "history" ? "Ukończone, pominięte i anulowane Działania pojawią się tutaj." : historicalActions.length ? "Zakończone Działania są zachowane w historii." : "Użyj przycisku Dodaj na dole, aby dodać pojedynczy krok."} action={actionFilter === "current" && historicalActions.length ? <Button onClick={() => setActionFilter("history")}><History />Pokaż historię</Button> : undefined} />}
      </section> : null}
