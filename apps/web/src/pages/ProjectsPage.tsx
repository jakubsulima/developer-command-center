...
import { Archive, ArrowRight, BookOpen, ChevronDown, FolderKanban, ListChecks, MoreHorizontal, Plus, RotateCcw, Target, Trash2 } from "lucide-react";
...
export function ProjectsPage() {
  ...
  return <AppShell addAction={{ label: "Nowy Projekt", shortLabel: "Projekt", ariaLabel: "Dodaj nowy Projekt", active: open, quickAdd: { mode: "project", pinnedToToday: false, draftKey: "projects-list" }, onClick: () => openCreate() }}>
    <PageHeading className="project-page-heading" title="Projekty" eyebrow={`${activeProjects} aktywne · stałe konteksty pracy`} action={<span className={`project-view-status project-view-status-${view}`} aria-label={`Widok: ${viewLabel}`}><span aria-hidden="true" />{viewLabel}</span>} />
    <p className="page-lead">Wszystkie Twoje projekty w jednym miejscu. Łącz je w kategorie — jeden projekt może należeć do kilku grup.</p>
    <div className="project-category-toolbar">
      <label className="project-category-filter"><span>Filtruj</span><span className="project-category-select"><select aria-label="Filtruj po kategorii" value={categoryFilter} onChange={(event) => setCategoryFilter(event.target.value)}><option value="">Wszystkie kategorie</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}<option value="uncategorized">Bez kategorii</option></select><ChevronDown aria-hidden="true" /></span></label>
      <details className="project-category-menu">
        <summary aria-label="Więcej opcji widoku" aria-haspopup="menu" title="Więcej opcji widoku"><MoreHorizontal /><span className="sr-only">Więcej opcji widoku</span></summary>
    ...
    {groups.map((group) => <section className="project-category-section" key={group.id}>
      ...
      return <Panel className={`${entityCardVariants()} project-card project-context-card entity-card`} key={project.id} data-navigation-card-id={`project-${project.id}`} tabIndex={-1}>
        ...
        <div className="project-card-categories">{categories.filter((category) => project.categoryIds?.includes(category.id)).map((category) => <span key={category.id} className="project-category-chip"><span style={{ backgroundColor: category.color }} />{category.name}</span>)}</div>
    ...
    })}</div> : <div className="category-empty"><FolderKanban /><p>{grouped ? "Nie ma jeszcze projektów w tej kategorii." : "Brak projektów pasujących do tego widoku."}</p></div>}
    </section>)}
