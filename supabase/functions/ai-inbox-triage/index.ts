...
import { configureAIProvider } from "../_shared/ai/config.ts";
...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    const config = configureAIProvider(CAPABILITY, (name) => env(name)); const provider = config.provider; const promptVersion = numberEnv("AI_INBOX_TRIAGE_PROMPT_VERSION", INBOX_TRIAGE_PROMPT_VERSION, 1, 1000); const now = new Date();
    ...
    if (!body.forceRefresh) {
      const { data: cached } = await admin.from("ai_inbox_triage_proposals").select("*").eq("workspace_id", body.workspaceId).eq("inbox_item_id", body.inboxItemId).eq("context_hash", hash).eq("prompt_version", promptVersion).eq("schema_version", SCHEMA_VERSION).eq("provider", provider.name).eq("model", provider.model).gt("cache_expires_at", now.toISOString()).order("created_at", { ascending: false }).limit(1).maybeSingle();
      if (cached) return jsonResponse(responsePayload(cached, true));
    }
    const providerRequest = { system: buildInboxTriageSystemPrompt(), input: context, jsonSchema: inboxTriageJsonSchema, schemaName: "inbox_triage", maxOutputTokens: numberEnv("AI_INBOX_TRIAGE_MAX_OUTPUT_TOKENS", 700, 300, 3000), timeoutMs: numberEnv("AI_INBOX_TRIAGE_TIMEOUT_MS", 25_000, 3_000, 60_000) };
    ...
    runId = await reserveAIRun(admin, {
      ...
      dailyLimit: numberEnv("AI_INBOX_TRIAGE_DAILY_LIMIT", 30, 1, 200), forceRefresh: body.forceRefresh,
    ...
    let providerResult;
    try { providerResult = await meter.complete(providerRequest); } catch (error) {
      if (!(error instanceof AIProviderError) || error.code === "PROVIDER_RATE_LIMITED" || (!error.retryable && error.code !== "PROVIDER_TIMEOUT")) throw error;
    ...
    try { proposal = validateAndFinalizeInboxTriage(parseProviderJson(providerResult.content), context); } catch {
      ...
      const repaired = await meter.complete({ ...providerRequest, system: `${providerRequest.system} Poprzedni wynik był niepoprawny. Zwróć poprawny JSON bez komentarza.` });
    ...
    const code = error instanceof AIBudgetError ? error.code : error instanceof AIProviderError ? error.code : error instanceof Error && error.message === "AI_NOT_CONFIGURED" ? "AI_NOT_CONFIGURED" : error instanceof Error && error.message === "invalid_request" ? "INVALID_REQUEST" : "INVALID_MODEL_OUTPUT";
    ...
    const status = code === "AI_NOT_CONFIGURED" ? 503 : code === "AI_BUDGET_EXCEEDED" || code === "AI_RATE_LIMITED" || code === "PROVIDER_RATE_LIMITED" ? 429 : code === "PROVIDER_TIMEOUT" ? 504 : code === "PROVIDER_REJECTED" ? 502 : code === "INVALID_REQUEST" ? 400 : 422;
